...
const getSensorIcon = (sensor) => {
  switch (sensor) {
    ...
    case "current":
      return <CurrentIcon />;
    case "packet_loss":
      return <PacketLossIcon />;
    case "bandwidth":
      return <BandwidthIcon />;
    case "fuel_level":
      return <FuelIcon />;
    case "oil_pressure":
      return <OilIcon />;
...
const DeviceStatus = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const processedDevices = devicesData
        .map((device) => {
          try {
            const deviceAlerts = processedAlerts.filter(
            ...
            return {
              ...
              alerts: {
                ...
                resolved: deviceAlerts.filter((alert) => alert.acknowledged)
                  .length,
              },
            };
          } catch (error) {
        ...
        setSensorData((prevData) => {
          ...
          for (const deviceId in newData) {
            ...
            const existingTimestamps = new Set(
              mergedData[deviceId].map((d) => d.rawTime)
            );
            const uniqueNewData = newData[deviceId].filter(
              (d) => !existingTimestamps.has(d.raw_timestamp)
            );
  ...
  return (
    <Box
      ...
      data-testid="device-status-grid"
    >
      <Box
        display="flex"
          ...
          return (
            <Grid item xs={12} sm={12} md={6} lg={4} key={device.id}>
              <Card
                ...
                onClick={() =>
                  (window.location.href = `/device-status/${device.id}`)
                }
              ...
              >
                <CardContent>
                  ...
                  >
                    ...
                    <Tooltip
                      title={
                        device.status_message || "No status message available"
                      }
                      arrow
                      placement="top"
                      componentsProps={{
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Location: <strong>{device.location}</strong> | Type:{" "}
                  ...
                  >
                    ...
                    {formatSensorData(device.id).length === 0 ? (
                      ...
                      <Box sx={{ height: 200 }} data-testid="sensor-data">
